...
parser.add_argument("--sigma", default=[0.1,1,10,50,100,500], type=list)
...
def train(training_data_loader, G_optimizer, D_optimizer, model, discr, criterion, epoch, sigma):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # break
    save_image(G_result.data, './checksample/output_r.png')
    save_image(input.data, './checksample/input_r.png')
    save_image(target.data, './checksample/gt_r.png')
...
def save_checkpoint(model, discr, epoch, ps, sigma):
    if epoch%10==0:
        ...
        if p>ps:
            ...
            model_out_path = "./checkpoint/" + "model_super_l05_"+str(opt.nEpochs)+"_"+str(sigma)+".pth"
